...
def display_select_mode():
    ...
    st.session_state.mode = st.sidebar.radio(
        # 「label_visibility="collapsed"」とすることで、ラジオボタンを非表示にする
            label="",
            options=[ct.ANSWER_MODE_1, ct.ANSWER_MODE_2], # ラジオボタンの選択肢
            label_visibility="collapsed"
    )
    st.sidebar.divider() # サイドバーの区切り線を表示
...
def display_initial_ai_message():
    ...
    with st.chat_message("assistant"):
        ...
        st.info("⚠️ 具体的に入力したほうが期待通りの回答を得やすいです。")
        # ここからsaidebarの内容を表示
        # 「社内文書検索」の機能説明
        ...
        if sub_choices:
            ...
            for sub_choice in sub_choices:
                ...
                if "page_number" in sub_choice:
                    ...
                    human_page = sub_choice['page_number'] + 1
                    st.info(f"{sub_choice['source']}（ページNo.{human_page}）", icon=icon)
                else:
                    # 「サブドキュメントのファイルパス」を表示
                    st.info(f"{sub_choice['source']}", icon=icon)
